            ...
            if(isContain(attribut[i].getName(), colone)){
                if(data[tab[j]]!=null){
                    ...
                    attribut[i].set(c,Integer.valueOf(data[tab[j]]));
                        ...
                        attribut[i].set(c,ca);
                    }if(attribut[i].getType()==MonDate.class){
                        MonDate d = new MonDate(data[tab[j]]);
                        attribut[i].set(c,d);
                    }
                }
...
public int[] getOrdreHafa(String[] col){
    ...
    public String[] requireValue(int[] tab)throws Exception{
        ...
        else if(attribut[tab[i]].getType()==MonDate.class){
            MonDate d = (MonDate) c.getClass().getMethod("get"+attr[tab[i]]).invoke(c);
    ...
    public String getRequete(String[] colonnes,String id)throws Exception{
        ...
        for(int i=1; i<attribut.length; i++){
            ...
            if(isContain(attribut[i].getName(), colonnes)){
                ...
                }else if(attribut[i].getType().equals(MonDate.class)){
                    MonDate d = (MonDate) this.getClass().getMethod("get"+attribut[i].getName().substring(0,1).toUpperCase()+""+attribut[i].getName().substring(1)).invoke(this);
        ...
        for(int i=1; i<attribut.length; i++){
            ...
            if(isContain(attribut[i].getName(), colonnes)){
                ...
                if(attribut[i].getType().equals(int.class) || attribut[i].getType().equals(double.class)){
                    ...
                    MonDate d = (MonDate) this.getClass().getMethod("get"+attribut[i].getName().substring(0,1).toUpperCase()+""+attribut[i].getName().substring(1)).invoke(this);
                    requete = requete + virgule+attribut[i].getName()+"=to_timestamp('"+d.getDate("yyyy-MM-dd HH:mm:ss")+"','YYYY-MM-DD HH24:MI:SS')";
